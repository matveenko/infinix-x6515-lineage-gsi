...
Скрипты **не устанавливают саму GSI автоматически**: разметка dynamic
partitions слишком опасна для слепой автоматизации. `INSTALL-RU.md` содержит
ручную установку LineageOS, а скрипты используются после её первого запуска
для проверки телефона, сохранения собственного vendor, сборки и прошивки фикса
яркости. Их точный порядок приведён в документе о яркости.
